...
function Boards() {
  ...
  useEffect(() => {
    ...
    // Mỗi khi url thay đổi ví dụ chúng ta chuyển trang thì cái location từ hook useLocation cũng thay đổi theo, đồng nghĩa hàm useEffect sẽ chạy lại và fetch lại API theo đúng page mới vì cái location.search đã nằm trong dependencies của useEffect
    // Gọi API lấy danh sách boards ở đây...
    // ...
    fetchBoardsAPI(location.search).then(updateStateData);
    // afterCreateNewBoard(location.search);
  }, [location.search]);
  const afterCreateNewBoard = () => {
    // Don gian fetch lai danh sach board
    fetchBoardsAPI(location.search).then(updateStateData);
  };
